...
class Revenue(models.Model):
    ...
    def is_projection_month(self):
        ...
        if self.financial_year.description == 'Year 1':
            months = dict(financial_year_months_extraction(self.financial_year.id))
            projection_month = self.product.projection_start
            invalid_projection_months = []
            for month in months.keys():
                if month < projection_month:
                    invalid_projection_months.append(month)
                    continue
                break
            if self.month in invalid_projection_months:
                return False
        return True
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_projection_month() is True:
